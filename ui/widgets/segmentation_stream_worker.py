...
class SegmentationStreamWorker(QThread):
    """Runs segmentation streaming off the main thread, frame by frame."""
    ...
    def run(self) -> None:
        if self._provider:
            self._app.segmentation.set_active_provider(self._provider)

        resolved_count = 0
        was_cancelled = False
        try:
            resolved_count = self._app.segmentation.detect_streaming(
